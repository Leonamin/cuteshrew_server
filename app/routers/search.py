...
def search_posts_by_user(user_id: Optional[int] = None, user_name: Optional[str] = None, start_post_id: Optional[str] = None, load_page_num: Optional[int] = 20, db: Session = Depends(database.get_db)):
    postingPreviewResponse = searchRepo.search_posts_by_user(user_id, user_name, start_post_id, load_page_num, db)
...
def search_comments_by_user(user_id: Optional[int] = None, user_name: Optional[str] = None, start_comment_id: Optional[str] = None, load_page_num: Optional[int] = 20, db: Session = Depends(database.get_db)):
    commentPreviewResponse = searchRepo.search_comments_by_user(user_id, user_name, start_comment_id, load_page_num, db)
